...
### Мастеру: обычная программа для Windows

Скачайте `BoardForge-Setup-1.0.exe` со страницы
[Releases](https://github.com/papsuevgeorgiy-ux/boardforge/releases),
запустите и трижды нажмите «Далее». Ничего больше ставить не нужно: Python,
библиотеки и справочник пород лежат внутри.

**Windows покажет синее окно «Windows защитила ваш компьютер».** Так бывает
с любой программой без платной цифровой подписи. Нажмите «Подробнее», затем
«Выполнить в любом случае».

После установки на рабочем столе появится ярлык **BoardForge**. Двойной клик —
откроется чёрное окно и следом браузер с интерфейсом. Окно закрывать нельзя,
пока работаете: программа живёт в нём. Закрыли окно — программа завершилась.

Что нужно знать сразу:

* **Печать.** Страница «В мастерскую» печатается из браузера по Ctrl+P —
  чертёж, раскрой, список в магазин и смета выходят на офисный лазерник как
  есть. Сохранение в PDF в этой сборке отключено намеренно, см.
  [Р34](docs/decisions.md).
* **Интернет не нужен.** Ни одна часть программы никуда не обращается.
* **Ваши проекты** — обычные файлы: сохраняются куда скажете и открываются
  обратно.

### Разработчику: из исходников
...
Собрать программу и установщик — конфигурации в `packaging/`:

```powershell
uv run pyinstaller packaging\boardforge.spec --noconfirm   # dist\BoardForge\
```

Установщик собирается [Inno Setup 6](https://jrsoftware.org/isdl.php): открыть
`packaging\installer.iss`, нажать F9. Обе конфигурации держат пути абсолютными
от корня проекта — и PyInstaller, и Inno Setup считают относительные пути от
каталога своего файла, а не от того, где вы стоите.
...
- у кубов по телу **каждой** грани проходит волосяная линия — это следствие
  конструкции, а не огрех рисунка: наклонные границы внутри столбца требуют
  двух резов, и каждый режет все три грани (Р23). Бесшовные кубы не строятся
