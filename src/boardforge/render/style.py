...
class Label:
    ...
    def clearance_mm(self) -> float:
        """Сколько места нужно вокруг точки подписи, чтобы буква не легла на шов.

        Выводится из высоты, а не задаётся отдельно, и это не экономия поля.
        Отдельный порог однажды уже разошёлся с буквой: он мерил габаритную
        рамку ячейки (11 мм), буква была 9 мм, и на ромбах кубов половина
        подписей выезжала на шов — рамка широкая, а по короткой диагонали
        места нет. Порог, выведенный из глифа, такого расхождения не допускает.
        """
...
BLUEPRINT = RenderStyle(
    ...
    background="#ffffff",
    repaint=_blueprint,
    label=Label(color="#000000", height_mm=9.0),
)
